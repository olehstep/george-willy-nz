(function ($) {
  ...
  */
  ...
  theme.scrollToRevealElement = function (el) {
    ...
    inViewTop = window.scrollY,
    inViewBot = window.scrollY + window.innerHeight - 50;
  ...
  theme.buildGalleryViewer = function (config) {
    ...
    $thumbContainer.find('.gallery-viewer__thumb:eq(' + currentIndex + ')').trigger('select').focus();
  ...
  theme.Navigation = {
    init: function init(options) {
      ...
      $mobileDrawer.on('click', '.navigation__tier-1 > .navigation__item > .navigation__children-toggle', function (evt) {
        ...
        // scroll container
        this.closest('.mobile-navigation-drawer').scrollTo({ top: 0, left: 0, behavior: 'smooth' });
      });
      ...
      if (options.proxyTier1Nav) {
        ...
        $(options.proxyTier1Nav).on('mouseenter', '.navigation__item--with-small-menu', function (evt) {
      ...
      productData.variants.forEach(variant => {
        if (variant.featured_media) {
          if (!optionNameMediaMap[variant.options[groupingOptionIndex]]) {
            optionNameMediaMap[variant.options[groupingOptionIndex]] = variant.featured_media.id;
          }
        }
      });
      // set option name on each image
      var currentMediaOptionName = '';
      $gallery.find('.slide, .thumbnail').each(function (index, item) {
      ...
      $(container).find('.product-media--video').each(function (index) {
        ...
        playerObj.play = function () {
          ...
          if (template) {
            ...
            this.container.style.paddingBottom = 100 / this.container.querySelector('video, iframe').getAttribute('aspect-ratio') + '%';
            $(this.container).closest('.slick-slider').slick('refresh');
          }
  ...
  theme.OptionManager = new function () {
    ...
    _.updatePrice = function (variant, $container) {
      ...
      if (variant) {
        ...
        var productData = _.getProductData($container.find('.product-form:first'));
        var cheapestVariant = productData.variants[0];
        var priceVaries = false;

        for (var i = 0; i < productData.variants.length; i++) {
          var v = productData.variants[i];
          if (v.price < cheapestVariant.price) {
            cheapestVariant = v;
          }
          if (v.price !== cheapestVariant.v) {
            priceVaries = true;
          }
        }
        ...
        if (cheapestVariant.compare_at_price > cheapestVariant.price) {
          ...
          $('<span class="was-price theme-money">').html(theme.Shopify.formatMoney(cheapestVariant.compare_at_price, theme.money_format)).appendTo($price);
        }
      }
  ...
  $(function ($) {
    ...
    $(document).on('load-product-form', '.product-form', function (e) {
          ...
          $productDetail.find('.dynamic-payment-button-template').each(function () {
            ...
            if (Shopify.PaymentButton) {
              ...
              _f = function f() {
                ...
                $productDetail.closest('.quickbuy-container').trigger('changedsize');
              };
      ...
      if ($availabilityDependentOptions.length) {
        $(this).on('variantChanged', function (evt, variant) {var _this2 = this;
          ...
          var _loop = function _loop(_optionIndex) {
            ...
            });};for (var _optionIndex = 0; _optionIndex < productData.options.length; _optionIndex++) {_loop(_optionIndex);
          }

          for (var _i = 0; _i < optionStatusMap.length; _i++) {var _loop2 = function _loop2(
            optionValue) {
              ...
              }).toggleClass('is-unavailable', !optionStatusMap[_i].values[optionValue]);};for (var optionValue in optionStatusMap[_i].values) {_loop2(optionValue);
            }
          }

          // product form may have changed height (e.g. variants customised to disappear). If in quick buy, resize container
          $(this).closest('.quickbuy-container').trigger('changedsize');
        });
      }
    ...
    $(document).on('variantImageSelected', '.gallery', function (e, data, isFirstRun) {
      ...
      if (!isFirstRun && $galleryCollageContainer.length) {
        var $active = $gallery.find('.slide[data-media-id="' + data.featured_media.id + '"]:first');
        if ($active.length) {
          if (collageGroupedImagesChanged) {
